...
for(var i=0;i<9;i++) {
	casas[i].addEventListener('click', (event) => {
		...
		if( (event.target.value=='_') && (vencedor=='_')) {
			...
			event.target.style.color='#F8F8FF'; //torna o valor da casa visível
...
b_reiniciar.addEventListener('click', (event) => {
	for(var i=0;i<9;i++) {
		...
		casas[i].style.color='#27408B'; //Torna o valor _ invisível
		casas[i].style.backgroundColor='#27408B'; //Torna o fundo branco
	}
...
var sortearJogador = function() {
	if(Math.floor(Math.random() * 2)==0) {
		...
		label_jogador.style.color='#F8F8FF'; //deixa o texto na cor vermelha
	}else{
		jogador = "X";//define o jogador X como atual
		label_jogador.innerText="X"; //exibe na página qual é o jogador atual
		label_jogador.style.color='#F8F8FF'; //deixa o texto na cor azul
	}
}
...
var trocarJogador = function() {
	if(jogador=='X') {
		...
		label_jogador.style.color='#F8F8FF';
